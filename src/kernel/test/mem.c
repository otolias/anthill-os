...
void _test_run_mem(void) {
    ...
    *(addr + 0x100) = 'a';
    ...
    // Free kernel page
    if (mem_kernel_free_page(kaddr_r.addr) != MEM_OK)
        panic("Kernel page de-allocation failed");

    // Try to free non-allocated page
    if (mem_kernel_free_page(0) != MEM_ERR_UNM)
    ...
    if (re_addr_r.err != MEM_OK || re_addr_r.addr != kaddr_r.addr)
    ...
    if (mem_user_map_page(tran_table, uaddr, paddr, MEM_RW) != MEM_OK)
